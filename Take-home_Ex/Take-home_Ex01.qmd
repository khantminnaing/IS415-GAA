...
```{r}
pacman::p_load(sf,st,arrow,lubridate,tidyverse,raster,tmap,ggplot2, patchwork,spatstat,spNetwork,gifski)
```
...
### 7.6 Analysis of Kernel Density Maps
...
```{r}
raster_tn_kde_scott <- raster(tn_kde_scott)
projection(raster_tn_kde_scott) <- CRS("+init=EPSG:3414 +units=km")
```
...
POIs_classes <- c("cafe", "food_court", "mall", "resturant", "supermarket", "hotel", "museum", "park", "school", "college","community_centre", "hospital","sports_centre", "library") 
...
tm_basemap(server = "Esri.WorldTopoMap") +
  ...
  tm_basemap(server = "OpenStreetMap") +
tm_shape(pg_lixels)+
  tm_lines(col="nkde_continuous", lwd = 1.5, palette =c("#ffd51c","#fd953b","#f02a75","#b62385","#021c9e"))+
tm_shape(pg_destination)+
  tm_dots(size=0.01) +
  tm_shape(pg_POIs)+
  tm_polygons(col="fclass", palette= c("#eff635","#e4c838","#ccb033","#99bf3e","#6ebf7c","#61adab","#5895d0","#5071ea","#4b49f5","#4225df","#3310a2"), id="fclass", title="POIs")+
  tm_borders(
  lwd = 0)
```
...
tm_basemap(server = "Esri.WorldTopoMap") +
  ...
  tm_basemap(server = "OpenStreetMap") +
tm_shape(tn_lixels)+
  tm_lines(col="nkde_continuous", lwd = 1.5, palette =c("#fafac3","#fd953b","#f02a75","#b62385","#021c9e"))+
tm_shape(tn_destination)+
  tm_dots(size=0.01) +
  tm_shape(tn_POIs)+
  tm_polygons(col="fclass", palette= c("#eff635","#e4c838","#ccb033","#99bf3e","#6ebf7c","#61adab","#5895d0","#5071ea","#4b49f5","#4225df","#3310a2"), id="fclass", title="POIs")+
...
```{r}
# choosing sample in times (every 3 hr)
...
```{r}
tn_destination$dropoff_hr_num <- as.numeric(tn_destination$dropoff_hr)

tn_dropoff_hr_num <- tn_destination$dropoff_hr_num

ggplot(data=tn_destination, 
                    aes(x=tn_dropoff_hr_num), bins = 24) + 
                    geom_bar() +
    scale_x_continuous(breaks = tn_dropoff_hr_num)
```

```{r}
# choosing sample in times (every 3 hr)
tn_sample_time <- seq(0, max(tn_destination$dropoff_hr_num), 1)

# calculating densities
tn_tnkde_densities <- tnkde(lines = tn_network,
                   events = tn_destination,
                   time_field = "dropoff_hr_num",
                   w = rep(1, nrow(tn_destination)), 
                   samples_loc = tn_samples,
                   samples_time = tn_sample_time, 
                   kernel_name = "quartic",
                   bw_net = 700, bw_time = 60,
                   adaptive = TRUE,
                   trim_bw_net = 900,
...
library(classInt)
library(viridis)
all_densities <- c(tn_tnkde_densities$k)
color_breaks <- classIntervals(all_densities, n = 10, style = "kmeans")

# generating a map at each sample time
all_maps <- lapply(1:ncol(tn_tnkde_densities$k), function(i){
  time <- tn_sample_time[[i]]
  
  tn_samples$tnkde_density <- tn_tnkde_densities$k[,i]
  map1 <- tm_shape(tn_samples) + 
  tm_dots(col = "tnkde_density", size = 0.01,
          breaks = color_breaks$brks, palette = viridis(10)) + 
    tm_layout(legend.show=FALSE, main.title = as.character(time), main.title.size = 0.5)
  return(map1)
...
```{r}
# creating a gif with all the maps
tmap_animation(all_maps, filename = "images/animated_tn.gif", 
               width = 1000, height = 1000, dpi = 300, delay = 50)

knitr::include_graphics("images/animated_tn.gif")
...
This analysis has explored the point patterns analysis using Grab's trajectory datasets. We have carried out three different approaches - Traditional Kernel Density Estimation (KDE), Network Constrained Kernel Density Estimation (NKDE) and Temporal Kernel Density Estimation (TKDE).
...
8.  Gimond (2023). Chapter 11 Point Pattern Analysis. Retrieved from https://mgimond.github.io/Spatial/index.html.

9.  González, J. A., & Moraga, P. (2022). An adaptive kernel estimator for the intensity function of spatio-temporal point processes.

10. Kam, T. S. (2022). R for Geospatial Data Science and Analytics. Retrieved from https://r4gdsa.netlify.app.

11. Krisp, J.M., Peters, S., Murphy, C.E., & Fan, H. (2009). Visual bandwidth selection for kernel density maps. Photogrammetrie - Fernerkundung - Geoinformation, 2009, 445--454.

12. Loader, C. (1999). Local Regression and Likelihood. Springer, New York.

13. O\'Sullivan, D., & Wong, D.W. (2007). A surface‐based approach to measuring spatial segregation. Geographical Analysis, 39, 147--168.

14. Pebesma, E., & Bivand, R. (2023). Spatial Data Science: With Applications in R (1st ed.). Chapman and Hall/CRC. https://doi.org/10.1201/9780429459016.

15. Rey, S.J., Arribas-Bel, D., & Wolf, L.J. (2023). Point Pattern Analysis. In: Geographic Data Science with python. CRC Press.

16. Scott, D.W. (1992). Multivariate Density Estimation. Theory, Practice and Visualization. New York: Wiley.

17. Shen, B., Xu, X., Li, J., Plaza, A., & Huang, Q. (2020). Unfolding spatial-temporal patterns of taxi trip based on an improved network kernel density estimation. ISPRS International Journal of Geo-Information, 9, 683.

18. Wilkin, J. (2020). Geocomputation 2020-2021 Work Book. University College London. Retrieved from https://jo-wilkin.github.io/GEOG0030/coursebook/analysing-spatial-patterns-iii-point-pattern-analysis.html.

19. Wolff, M., & Asche, H. (2009). Towards geovisual analysis of crime scenes -- a 3D crime mapping approach. Advances in GIScience, 429--448.

20. Xie, Z., & Yan, J. (2008). Kernel density estimation of traffic accidents in a network space. Computers, Environment and Urban Systems, 32, 396--406.

21. Yuan, Y., Qiang, Y., Bin Asad, K., & Chow, T. E. (2020). Point Pattern Analysis. In J.P. Wilson (Ed.), The Geographic Information Science & Technology Body of Knowledge (1st Quarter 2020 Edition). DOI: 10.22224/gistbok/2020.1.13.
