...
We can try to plot both results together on the same plane for better comparison of the mean center and median center as well.
...
destination_sd <- sqrt(sum((origin_xy[,1] - destination_xy[1])^2 + (origin_xy[,2] - destination_xy[2])^2) / nrow(origin_xy))

origin_sd
destination_sd
```

From the results, the origin and destination standard distances are **10187.88** and **23452.37**, respectively. Hence, it appears that destination points are more dispersed than the origin points.

::: {.callout-tip title="Reflection"}
However, it would be challenging to discern why the destination points are more dispersed without further analysis. Further analysis would be needed to determine the factors contributing to the increased dispersion of destination points. Since it is out of scope for this exercise, we will not explore any further.
:::

#### 6.2.3 Plotting Standard Distances

In this section, we will create a bearing circle using the standard distance values we have calculated earlier.  
...
Kernel Destiny Estimation (KDE) produces a surface (raster) that details the estimated distribution of point events over observation window. Each cell within the KDE layer contains a value that is the estimated density at that location (). Hence, this approach is also known as local density approach. To create the KDE layer, a localised density for small subsets of the observation window is computed. However, these subsets overlap one another in each iteration, creating a **moving window**, defined by a **kernel**.
...
1\. Floch, J.-M., Marcon, E., Puech, F.: Spatial distribution of points. In: Bellefon, M.-P. de (ed.) Handbook of Spatial Analysis : Theory and Application with R. pp. 72--111. Insee-Eurostat.
