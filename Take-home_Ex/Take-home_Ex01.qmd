...
```{r}
pg_densities <- densities <- nkde(pg_network, 
...
#### 8.4.1 **Visualising NetKDE densities**
...
```{r}
pg_samples$nkde_density <- pg_densities
pg_lixels$nkde_density <- pg_densities
```

Similar to what we did in KDE, we will rescle the density values.

```{r}
pg_samples$nkde_density <- pg_samples$nkde_density*1000
pg_lixels$nkde_density <- pg_lixels$nkde_density*1000
```

Now we can plot the NetKDE map using `tmap`.
...
tm_shape(pg_lixels)+
  tm_lines(col="nkde_density", palette =c("#fafac3","#fd953b","#f02a75","#b62385","#021c9e"))+
tm_shape(pg_destination)+
  tm_dots(size=0.005)
```
